(function($) {
	...
	$.zxxbox = function(elements, options) {
		...
		$.isFunction(s.onshow) && s.onshow();
		//尺寸
		$.zxxbox.setSize();
		//定位
		$.zxxbox.setPosition();

		s.fix && $.zxxbox.setFixed();
		s.drag ?  $.zxxbox.drag() : $(window).resize(function() { $.zxxbox.setPosition();	});
		!s.bar ? $.zxxbox.barHide() : $.zxxbox.barShow();
		!s.bg ? $.zxxbox.bgHide() : $.zxxbox.bgShow();
		!s.btnclose ? $.zxxbox.closeBtnHide() : $.o.clo.click(function() { $.zxxbox.hide(); return false; });
		s.bgclose && $.zxxbox.bgClickable();	
		s.delay > 0 && setTimeout($.zxxbox.hide, s.delay);	
	};
	$.extend($.zxxbox, {
		getSize: function(o) {
			//获取任意元素的高宽
			var w_h = {}, o_new = o.clone();
			$('<div id="zxxobxWrapClone" style="position:absolute;left:-6000px;"></div>').appendTo("body").append(o_new);
			w_h.w = $("#zxxobxWrapClone").width();
			w_h.h = $("#zxxobxWrapClone").height();
			$("#zxxobxWrapClone").remove();
			return w_h;
		},
		setSize: function() {
		...
		remind: function(message, callback, options) {
			...
			$.zxxbox(element, options);
			if(!options.btnconfirm) $('#zxxSubmitBtn').hide();
		...
		ajax: function(uri, params, options) {
			...
			if (uri) {
				$.zxxbox.loading({title:options.title});
