...
function GetTopFourGoods() {
    ...
    if(num==0){
        ...
        var empval="<div style='text-align:center; width:100%;line-height:30px;margin-top:5px'>The shopping cart is empty.</div><b><a href='";
        empval+=$("#carts").attr("emptycar");
        empval+= "' style=\"color:#1F8AEA\">View Shopping Cart &gt;&gt;</a></b>";
...
function datas(data,counts,tot){
    ...
    html+='  <span id="topPrice"><font><font>Total： $'+(tot/huil).toFixed(2)+' </font></font></span>';
    html+='   <a href="'+$("#carts").attr("emptycar")+'"><font><font>View Shopping Cart &gt;&gt;</font></font></a>';
    html+='</div>';
    //alert('ss'+html);
    return html;
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.result == true) {
                ...
                $("#username").html("Welcome,<a href='"+userurl+"'>" + data.username + "</a>");
                $("#regOrSignOut").html("<div ><a href='javascript:void(0)' onclick='SignOut()' >Sign Out</a><div>");
            }
            else {
                if (data.username == null || data.username == "") {
                    $("#username").html("<a href='"+userurl+"'> Sign In</a>");
                    $("#regOrSignOut").html("<a href='"+regurl+"'>Register</a>");
                }
                else {
                    $("#username").html("Welcome," + data.username + "&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a href='"+userurl+"'>Sign In</a>");
                    $("#regOrSignOut").html("<a href='"+regurl+"'>Register</a>");
                }
            }
        }
...
function cartNum(){
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result == true) {
                $("#goodsNum").html("<b>Cart: </b> <font id=\"newNum\">" + data.data + "</font> item(s)");
...
function cartNumCookie(){
    var shopcart =$.cookie('shopcart');
    if(shopcart==null){
        $("#goodsNum").html("<b>Cart: </b> <font id=\"newNum\">0 </font>item(s)");
    }else{
        shopcart=shopcart.split(",");
        var ls=shopcart.length;
        $("#goodsNum").html("<b>Cart: </b> <font id=\"newNum\">" + ls/2 + " </font> item(s)");
    }
}
//登出
function SignOut() {
