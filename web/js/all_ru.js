...
$(document).ready(function () {
    var user=$("#username a").html();
    if(user=="Вход"){
        cartNumCookie();
    }
    login();
 
    sh('Tool-menu','toolmenu');
    sh('head-menu','helpmenu');
    sh('headaccountMenu','acuntmove');
    sh('catemenudrop','');
    showcat('carts','movecart');
});
function showcat(name,classname){
    $("#"+name).hover(function(){
        $(this).find("dd").show();
...
function GetTopFourGoods() {
    var emptyhtml="<div style='text-align:center; width:100%;line-height:30px;float:left;margin-top:5px;'><img src='web/images/user/loadingb.gif' /></div>";
    $("#ShopingCart").html(emptyhtml);
    var num= parseInt($("#newNum").html());
    if(num==0){
		  
        var empval="<div style='text-align:center; width:100%;line-height:30px;margin-top:5px'>Корзина пуста..</div><b><a href='";
        empval+=$("#carts").attr("emptycar");
        empval+= "' style=\"color:#1F8AEA\">Перейти в корзину &gt;&gt;</a></b>";
        $("#ShopingCart").html(empval);	 
        datas();
    }else{
        var shopcaturl=$("#carts").attr("url");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.result) {
                    ...
                    var data=  datas(data.data,data.count,data.tot);
                    $("#ShopingCart").html(data);
                }
                else {
                    var empurl=$("#carts").attr("emptycar");
                    $("#ShopingCart").html("<div style='text-align:center; width:100%;line-height:30px;margin-top:5px'>The shopping cart is empty.</div><b><a href='"+empurl+"' style=\"color:#1F8AEA\">View Shopping Cart &gt;&gt;</a></b>");
                }
            }
        }); 
    }    
...
function deleshopcart(id){
    var urls=$('#carts').attr("delecshopcat");
    $.ajax({
        url:urls,
        type:"POST",
        dataType:"json",
            ...
            if(data.result==true){
                ...
                if(shopcart.length==2){
                    ...
                    for(var i=0;i<shopcart.length;i++){
                        if(id==shopcart[i]){
                            if((i+2)==shopcart.length){
                                shopcart.splice(i-1,3,shopcart[i-1]);
                            }else{
                                shopcart.splice(i,3,shopcart[i+2]);
                            }
                        }
                    }
...
function datas(data,counts,tot){
    var huil=$("#carts").attr("huil");
    var url=$("#carts").attr("idhref");
    ...
    for(var i=0;i<counts;i++){
        ...
        html+='<div class="dgxx">';
        html+='<dl>    ';
        html+='<dt style="background: none; border: none; padding: 0; margin: 0; margin-right: 10px;">  ';      
        html+='	<a href="" style="padding: 0;"> '; 
        html+='	<img src=" '+data[i]["pic"]+' " alt="">';
        html+='	</a>'; 
        html+='</dt>'; 
        html+='	<dd> ';  
        html+='		<h4> <a href=" '+url+data[i]["pid"]+' "><font><font>'+data[i]["name"]+'</font></font></a></h4> ';    
        html+='		<span><font><font>Price：</font></font><em><font><font> $'+(data[i]["price"]/huil).toFixed(2)+'  </font></font></em></span>';
        html+='		<span><font><font>Qty：</font></font><em><font><font> '+data[i]["number"]+' </font></font></em></span>    ';
        html+='	</dd>';
        html+='</dl>';
        html+='<div class="deleat">  ';
        html+='<a href="javascript:deleshopcart('+data[i]["pid"]+')">';
        html+='		<span style="cursor:pointer" id="">   ';
        html+='		<img src="web/images/head/delete.png" width="70" height="28" alt="">';
        html+='		</span>';
        html+='	</a>';
        html+='</div>';
    ...
    html+='<div class="carttol">  ';  
    html+='  <span id="topPrice"><font><font>Сумма： $'+(tot/huil).toFixed(2)+' </font></font></span>';
    html+='   <a href="'+$("#carts").attr("emptycar")+'"><font><font>Перейти         в корзину &gt;&gt;</font></font></a>';
    html+='</div>';
    //alert('ss'+html);
    return html;
}
...
function login() { 
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            cartNum(); 
            if (data.result == true) {
                var userurl=$("#urls").attr("userurl");
                $("#username").html("Добро пожаловать,<a href='"+userurl+"'>" + data.username + "</a>");
                $("#regOrSignOut").html("<div ><a href='javascript:void(0)' onclick='SignOut()' >Выход</a><div>");
            }
            else {
                if (data.username == null || data.username == "") {
                    $("#username").html("<a href='"+userurl+"'> Вход</a>");
                    $("#regOrSignOut").html("<a href='"+regurl+"'>Регистрация</a>");
                }
                else {
                    $("#username").html("Добро пожаловать," + data.username + "&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a href='"+userurl+"'>Вход</a>");
                    $("#regOrSignOut").html("<a href='"+regurl+"'>Регистрация</a>");
                }
            }
        }
...
function cartNum(){
    var loginShopCatMun=$('#carts').attr('loginShopCatMun');
    $.ajax({
        url: loginShopCatMun,
        dataType: "json",
        cache: false,
        success: function (data) {
            if (data.result == true) {
                $("#goodsNum").html("<b>Корзина: </b> <font id=\"newNum\">" + data.data + "</font> item(s)");
...
function cartNumCookie(){
    var shopcart =$.cookie('shopcart');
    if(shopcart==null){
        $("#goodsNum").html("<b>Корзина: </b> <font id=\"newNum\">0 </font>item(s)");
    }else{
        shopcart=shopcart.split(",");
        var ls=shopcart.length;
        $("#goodsNum").html("<b>Корзина: </b> <font id=\"newNum\">" + ls/2 + " </font> item(s)");
    }
}
//登出
function SignOut() {
    var userout=$("#urls").attr("userout");
    $.ajax({
        url: userout,
        dataType: "json",
        cache: false,
        ...
        success: function (data) {
            if (data.result == true) {
                var userurl=$('#username').attr('url');
                var regurl=$('#regOrSignOut').attr('url');
                var indexurl=$('#urls').attr('indexurl');
                $("#username").html("<a href='"+userurl+"'>Вход</a>");
                $("#regOrSignOut").html("<a href='"+regurl+"'>Регистрация</a>");
...
function rightkeys(object) {
    ...
    if (keyvalue != "") {
        $("#Keys").val(keyvalue);
        window.location.href = surl+"?keyword=" + keyvalue;
    }
}
 ...
 * close 关闭
 */
function drag(mask,drags,top,close){

    var oWin = document.getElementById(drags);
	
    var oLay = document.getElementById(mask);	
	
    var oClose = document.getElementById(close);
	
    var oH2 = document.getElementById(top);
	
    var bDrag = false;
    var disX = disY = 0;
    var hlayh=document.body.offsetHeight;
    var wintop=getPageScroll();
    oLay.style.display = "block";
    oLay.style.height=hlayh+"px";
    oWin.style.marginTop =wintop+"px";
    ...
    document.onmousemove = function (event)
    {
        ...
        var maxL = document.body.offsetWidth - oWin.offsetWidth;
        var maxT = document.body.offsetHeight - oWin.offsetHeight;		
        iL = iL < 0 ? 0 : iL;
        iL = iL > maxL ? maxL : iL; 		
        iT = iT < 0 ? 0 : iT;
        iT = iT > maxT ? maxT : iT;
...
$(document).ready(function() {
    $("[name='soncate']").mouseleave(function() {
        ...
        $(this).find("span").append("<img src=\"./web/images/head/xy.png\" width=\"12\" height=\"12\" alt=\"\" class=\"hid\" />");
    });
