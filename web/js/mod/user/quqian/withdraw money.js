...
$(document).ready(function () {
    paymentType(0, 1, 1);
    $("#two1").click(function () {//PayPal
        paymentType(0, 1, 1);
    });
    $("#two2").click(function () {//Western Union
        paymentType(1, 1, 0);
    });
    $("#two3").click(function () {//MoneyGram
        paymentType(1, 2, 0);
    });
    $("#two4").click(function () {//DBS
        paymentType(1, 4, 1);
    });
    $("#two5").click(function () {//WEBMONEY
        paymentType(1, 5, 0);
    });
    ...
    function paymentType(PaymentMode, PaymentTypeID, Currency) {
        $.post("https://account.yoybuy.com/en/getpaymentinfo", {
        ...
        function (data, textStatus) {
            ...
            if (array != null) {
                ...
                $("#Rate").val(array[4]);
                $("#curch").val(array[3]);
                var fee = parseFloat(array[1]) * 100;
                var surch = parseFloat(array[2]);
                $("#paymentInfo").html("Money will be converted to " + array[3] + ". " + array[0] + " fee is " + fee + "% + " + surch + " " + array[3]);
            }
            else {
            //alert("Failure to obtain payment details, please try again");            
            }
        });
...
function SubmitAddRefundInfo(type, abtype) {
    ...
    //获取录入数据 结束
    if (abtype == "pp") {
        if ($("#ppApplyMoneyValidation").text() != "√") {
    ...
    if (confirm("Are you sure that all the information is correct?")) {
        ...
        $.post("https://account.yoybuy.com/en/submitrefund",
        {
            "ApplyMoney": applyMoney, 
            "UserName": UserName, 
            "CardNo": CardNo, 
            "Connect": Connect, 
            "DrawbackType": paymentType, 
            "EmailAddresss": Email, 
            "ManName": ManName, 
            "Remark": Remark, 
        ...
        function (data, textStatus) {
            ...
            if (obj3.Success) {
                ...
                $.colorbox({
                    ...
                    , 
                    onClosed: function () {
                        window.location = "https://account.yoybuy.com/en/myrefundrecords";
                    }
                });
            }
            else {
