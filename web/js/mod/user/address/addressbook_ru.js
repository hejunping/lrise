...
function check_addressName(object) {
    if (!is_forbid($(object).val()) || trimTxt($(object).val()) == "") {
        if (!is_forbid($(object).val())) {
            $(object).parent().next("div").text("Имя пользователя не может содержать незаконный характер!");
        }
        if (trimTxt($(object).val()) == "") {
            $(object).parent().next("div").text("Пожалуйста, введите ваше имя!");
        }
        return false;
    } else {
...
function check_tel(object) {//楠岃瘉鐢佃瘽
    if (trimTxt($(object).val()) == "") {
        $(object).parent().next("div").text("Пожалуйста, введите ваш номер телефона!");
        return false;
    } else {
        $(object).parent().next("div").text("");
        return true;
...
function check_address(object) {
    if (trimTxt($(object).val()) == "") {
        $(object).parent().next("div").text("Пожалуйста, введите ваш  адрес!");
        return false;
    } else {
        $(object).parent().next("div").text("");
        return true;
        ...
        if (keymessage.init(object)) {
            if ($(object).val() == "") {
                switch ($(object).attr("name")) {
                    case "Provinces":
                        $(object).parent().next("div").text("Пожалуйста, введите ваш  провинции /  чжоу .");
                        break;
                    case "City":
                        $(object).parent().next("div").text("Пожалуйста, введите ваш  город.");
                        break;
                    default:
                        $(object).parent().next("div").text("");
                        break;
...
function check_Country(object) {
    if ($(object).val() == 0) {
        $(object).parent().next("div").text(" Пожалуйста, введите ваш  страны");
        return false;
    } else {
        $(object).parent().next("div").text("");
