...
function check_addressName(object) {
    if (!is_forbid($(object).val()) || trimTxt($(object).val()) == "") {
        if (!is_forbid($(object).val())) {
            $(object).parent().next("div").text("The user name can not include illegal characters!");
        }
        if (trimTxt($(object).val()) == "") {
            $(object).parent().next("div").text("Please enter in your name!");
        }
        return false;
    } else {
...
function check_tel(object) {//楠岃瘉鐢佃瘽
    if (trimTxt($(object).val()) == "") {
        $(object).parent().next("div").text("Please enter in your phone number");
        return false;
    } else {
        $(object).parent().next("div").text("");
        return true;
...
function check_address(object) {
    if (trimTxt($(object).val()) == "") {
        $(object).parent().next("div").text("Please enter in your address");
        return false;
    } else {
        $(object).parent().next("div").text("");
        return true;
        ...
        if (keymessage.init(object)) {
            if ($(object).val() == "") {
                switch ($(object).attr("name")) {
                    case "Provinces":
                        $(object).parent().next("div").text("Please enter your Provinces/state.");
                        break;
                    case "City":
                        $(object).parent().next("div").text("Please enter in your city");
                        break;
                    default:
                        $(object).parent().next("div").text("");
                        break;
...
function check_Country(object) {
    if ($(object).val() == 0) {
        $(object).parent().next("div").text("Please select your country");
        return false;
    } else {
        $(object).parent().next("div").text("");
