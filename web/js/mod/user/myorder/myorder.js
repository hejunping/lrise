...
function LoadOrderData(page) {
    ...
    if (GetOrderDataCache(page) == false) {
        ...
        $.ajax({
            url: "http://order.yoybuy.com/order/myorderdata",
            cache: false,
...
$(document).ready(function () {
    ...
    $("#stateOrder").change(function () {
        var num = $("#stateOrder option:selected").attr("name");
...
function Jump(num) {
    ...
    var jumpUrl;
    if (num == 1) {
        jumpUrl = url.replace("_Url_", "All");

    }
    if (num == 2) {
        jumpUrl = url.replace("_Url_", "Procuring");

    }
    if (num == 3) {
        jumpUrl = url.replace("_Url_", "Arrived");

    }
    if (num == 4) {
        jumpUrl = url.replace("_Url_", "Refund");
    ...
    if (num == 6) {
        jumpUrl = url.replace("_Url_", "AwaitPurchase");
    }
    if (num == 7) {
        jumpUrl = url.replace("_Url_", "Inprocess");
    }
    if (num == 8) {
        jumpUrl = url.replace("_Url_", "AwaitPayment");
    }
    if (num == 9) {
        jumpUrl = url.replace("_Url_", "Bought");
    }
    if (num == 10) {
        jumpUrl = url.replace("_Url_", "Arrived");
    }
    if (num == 11) {
        jumpUrl = url.replace("_Url_", "Storing");
    }
    if (num == 12) {
        jumpUrl = url.replace("_Url_", "Abandon");
    }
    if (num == 13) {
        jumpUrl = url.replace("_Url_", "Message");
    }
