...
function GetTopFourGoods() {
    ...
    if(num==0){
        ...
        var empval="<div style='text-align:center; width:100%;line-height:30px;margin-top:5px'>购物车是空的.</div><b><a href='";
        empval+=$("#carts").attr("emptycar");
        empval+= "' style=\"color:#1F8AEA\">查看购物车 &gt;&gt;</a></b>";
...
function datas(data,counts,tot){
    ...
    html+='  <span id="topPrice"><font><font>总计： $'+(tot/huil).toFixed(2)+' </font></font></span>';
    html+='   <a href="'+$("#carts").attr("emptycar")+'"><font><font>查看购物车&gt;&gt;</font></font></a>';
    html+='</div>';
    //alert('ss'+html);
    return html;
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.result == true) {
                ...
                $("#username").html("欢迎,<a href='"+userurl+"'>" + data.username + "</a>");
                $("#regOrSignOut").html("<div ><a href='javascript:void(0)' onclick='SignOut()' >退出</a><div>");
            }
            else {
                if (data.username == null || data.username == "") {
                    $("#username").html("<a href='"+userurl+"'> 登录</a>");
                    $("#regOrSignOut").html("<a href='"+regurl+"'>注册</a>");
                }
                else {
                    $("#username").html("欢迎," + data.username + "&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a href='"+userurl+"'>登录</a>");
                    $("#regOrSignOut").html("<a href='"+regurl+"'>注册</a>");
                }
            }
        }
...
function cartNum(){
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result == true) {
                $("#goodsNum").html("<b>购物车: </b> <font id=\"newNum\">" + data.data + "</font> item(s)");
...
function cartNumCookie(){
    var shopcart =$.cookie('shopcart');
    if(shopcart==null){
        $("#goodsNum").html("<b>购物车: </b> <font id=\"newNum\">0 </font>item(s)");
    }else{
        shopcart=shopcart.split(",");
        var ls=shopcart.length;
        $("#goodsNum").html("<b>购物车: </b> <font id=\"newNum\">" + ls/2 + " </font> item(s)");
    }
}
//登出
function SignOut() {
