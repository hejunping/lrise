    ...
    if (isNaN(cost)) {
        ...
        { "fireghtType": type, "countryId": countryId, "goodsCost": cost, "weight": googsWeight },
         function (data, textStatus) {
             ...
             $("#Dazhehou").html(FormatNum((parseFloat(TotalCost) + parseFloat(GoodsAmount) + parseFloat(homeFeright)), 2));
             ...
             $("#baoguanfei").html(FormatNum(CNYtoUSDConstr(data.ClearanceCharge), 2));
             $("#Yunfeitxt").html(FormatNum(CNYtoUSDConstr(data.DiscountFreight), 2));
             $("#packInfo").html("&nbsp;&nbsp; ( Weight+" + data.PackCharge + " % )");

             var producttxt = FormatNum(CNYtoUSDConstr($("#GoodsAmount").val()), 2);
             $("#Producttxt").html(FormatNum((parseFloat(homeFeright) + parseFloat(producttxt)), 2));
