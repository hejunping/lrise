    ...
    function thumb($image,$is_save=true,$suofang=0,$type='',$maxWidth=500,$maxHeight=500,$interlace=true){
       ...
       $info  =pic::getImageInfo($image);
         if($info != false) {
            ...
            if ($suofang==1) {
				$length=-17;
				$mlen=strlen($image)+$length-1;
				$mima=substr($image,$mlen,1);
				if($mima!='/'){$length=-18;}
				$thumbname01 = substr_replace($image,"big.".$type,$length);		//大头像
				$thumbname02 = substr_replace($image,"middle.".$type,$length);		//小头像
				$thumbname03 = substr_replace($image,"small.".$type,$length);	
                ...
                imagecopyresampled($thumbImg01,$thumbImg,0,0,$_POST['x'],$_POST['y'],150,150,$_POST['w'],$_POST['h']);
                ...
                $thumbImg03 = imagecreatetruecolor(60,60);
                imagecopyresampled($thumbImg03,$thumbImg,0,0,$_POST['x'],$_POST['y'],60,60,$_POST['w'],$_POST['h']);
				...
				if($is_save == false){											//缩略图覆盖原图，缩略图的路径还是原图路径
					...
					$thumbname03 = substr_replace($image,"04.".$type,$length);	//缩略图与原图同时存在，
					$imageFun($thumbImg,$thumbname03,100);
					imagedestroy($thumbImg);
					imagedestroy($srcImg);
