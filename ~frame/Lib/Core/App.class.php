...
class App {
    ...
    static public function init() {
        ...
        $group   =  defined('GROUP_NAME')?GROUP_NAME.'/':'';//修改---移到此处---
        if(C('TMPL_DETECT_THEME')) {// 自动侦测模板主题 TMPL_SWITCH_ON TMPL_DETECT_THEME
            $t = C('VAR_TEMPLATE');
            if (isset($_GET[$t])){
                $templateSet = $_GET[$t];
            ...
            // 主题不存在时仍改回使用默认主题
            if(!is_dir(TMPL_PATH.$group.$templateSet))//修改---加了.$group.---
                $templateSet = C('DEFAULT_THEME');
            cookie('think_template',$templateSet);
        }
        /* 模板相关目录常量 */
        define('THEME_NAME',   $templateSet);                  // 当前模板主题名称
        //$group   =  defined('GROUP_NAME')?GROUP_NAME.'/':'';	//修改---移到上面---针对此项目做分组的结构--
    ...
    static public function exec() {
        ...
        if(!$module) {
            if(function_exists('__hack_module')) {
                ...
                // 是否定义Empty模块
                $module = defined('GROUP_NAME') ? A(GROUP_NAME.'/Empty') : null;
                if(!$module) $module = A('Empty');
                if(!$module){
